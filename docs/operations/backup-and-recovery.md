...
Este guia organiza a proteção e a recuperação de um cluster sem depender de uma única ferramenta. Ele detalha os itens do [guia de operação contínua](../guides/operations.md) e deve ser adaptado à criticidade, aos dados e às dependências de cada ambiente.
...
Snapshots CSI também podem permanecer no mesmo backend do PV original. Durabilidade e portabilidade dependem do driver, da `VolumeSnapshotClass` e do provedor. Consulte o [guia do Longhorn](../k8s/extensions/longhorn.md) e teste a combinação exata adotada.
...
Recuperar objetos Secret do etcd não garante que a fonte externa, as identidades e as credenciais de bootstrap ainda funcionem. Para soluções como o [Infisical](../strategies/secrets/infisical.md), documente:
...
- [ ] Executar os fluxos críticos definidos na [prontidão de workloads](../k8s/workloads/production-readiness.md).
...
- [Backup, atualização e remoção no K3s](../k8s/k3s/operations/cluster-maintenance.md)
- [Longhorn](../k8s/extensions/longhorn.md)
- [Segredos GitOps com Infisical](../strategies/secrets/infisical.md)
- [Prontidão de workloads](../k8s/workloads/production-readiness.md)
