# Runbook de manutenção e mudanças
...
- [guia de operação contínua](../guides/operations.md);
- [prontidão de workloads para produção](../k8s/workloads/production-readiness.md);
- [ciclo de vida de imagens e rollouts](../strategies/deployment/image-lifecycle.md);
- [observabilidade e alertas](observability-and-alerting.md);
- [backup e recuperação](backup-and-recovery.md);
- [manutenção do cluster K3s](../k8s/k3s/operations/cluster-maintenance.md);
...
As rotinas abaixo são um índice. Selecione os itens aplicáveis no [guia de operação contínua](../guides/operations.md) e registre somente resultado, exceções, evidências e ações pendentes neste runbook; não replique todos os checkboxes a cada execução.

| Frequência inicial | Foco da revisão | Guias para critérios e validação |
| --- | --- | --- |
| Contínua ou diária | Saúde do cluster e dos workloads, sinais externos, capacidade, coleta de telemetria, alertas e último backup esperado | [Observabilidade e alertas](observability-and-alerting.md), [backup e recuperação](backup-and-recovery.md) e [guia contínuo](../guides/operations.md) |
| Semanal | Tendências, alertas ruidosos ou recorrentes, certificados, divergência de imagens e presença dos backups externos | [Observabilidade e alertas](observability-and-alerting.md), [ciclo de vida de imagens](../strategies/deployment/image-lifecycle.md) e [backup e recuperação](backup-and-recovery.md) |
| Mensal ou por janela | Atualizações disponíveis, compatibilidade, capacidade, requests e limits, retenção, responsáveis e prontidão para drenar nós | [Prontidão de workloads](../k8s/workloads/production-readiness.md), [ciclo de vida de imagens](../strategies/deployment/image-lifecycle.md) e [manutenção do K3s](../k8s/k3s/operations/cluster-maintenance.md) |
| Trimestral ou conforme a criticidade | Restauração cronometrada, teste ponta a ponta de alertas, perda de componentes, acessos e dependências fora de suporte | [Backup e recuperação](backup-and-recovery.md), [observabilidade e alertas](observability-and-alerting.md) e [guia contínuo](../guides/operations.md) |
...
- a [validação pós-instalação](../k8s/k3s/operations/checklist.md) e a [prontidão dos workloads](../k8s/workloads/production-readiness.md) não possuem pendências bloqueantes aplicáveis.

### Backup e prontidão de restauração

Ter um arquivo de backup não prova que o ambiente pode ser recuperado. Use o [guia de backup e recuperação](backup-and-recovery.md) para distinguir snapshot do datastore, volumes, backups próprios das aplicações, configurações, credenciais e token do K3s.
...
Registre separadamente versões de charts, aplicações, imagens, CRDs e controllers. Siga o [ciclo de vida de imagens e rollouts](../strategies/deployment/image-lifecycle.md) para identificar tag, digest, origem, homologação e artefato anterior.
...
Consulte os critérios detalhados de [prontidão de workloads](../k8s/workloads/production-readiness.md), [observabilidade e alertas](observability-and-alerting.md) e [manutenção do K3s](../k8s/k3s/operations/cluster-maintenance.md).
