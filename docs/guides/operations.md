...
**Procedimento:** [prontidão de workloads para produção](../k8s/workloads/production-readiness.md). **Evidência mínima:** owner e criticidade registrados, manifesto ou revisão versionada e resultado das validações aplicáveis.
...
**Procedimento:** [ciclo de vida de imagens de containers](../strategies/deployment/image-lifecycle.md). **Evidência mínima:** tag e digest desejados e observados, origem da mudança, avaliação de compatibilidade, homologação e referência de rollback.
...
**Procedimento:** [prontidão de workloads para produção](../k8s/workloads/production-readiness.md). **Evidência mínima:** semântica de cada probe, tempos medidos e resultados dos testes de inicialização, indisponibilidade, travamento e encerramento.
...
**Procedimento:** [observabilidade e alertas](../operations/observability-and-alerting.md). **Evidência mínima:** inventário de sinais e targets, dashboards ou consultas, retenção, teste de indisponibilidade e confirmação de monitoramento externo.
...
**Procedimento:** [observabilidade e alertas](../operations/observability-and-alerting.md). **Evidência mínima:** regra, owner, severidade, runbook, destino e resultado datado de um teste ponta a ponta.
...
**Procedimento:** [backup e recuperação](../operations/backup-and-recovery.md). **Evidência mínima:** matriz de ativos, RPO/RTO, última execução válida, cópia fora do domínio de falha e resultado cronometrado do último restore drill.
...
Registre as execuções com o [runbook de manutenção e mudanças](../operations/maintenance-runbook.md). Os itens abaixo definem a cobertura; o runbook guarda responsável, resultado, evidência, exceções e ações pendentes sem duplicar o checklist inteiro.
...
- [ ] [Certificados](../k8s/extensions/cert-manager.md) e credenciais próximos do vencimento foram identificados.
- [ ] Não surgiram workloads com [tags flutuantes, imagens sem inventário ou versões divergentes](../strategies/deployment/image-lifecycle.md) do estado desejado.
- [ ] [Backups externos esperados](../operations/backup-and-recovery.md) estão presentes dentro da retenção definida.

### Mensal ou por janela de manutenção

- [ ] Novas versões de K3s, charts, controllers, operators e [imagens](../strategies/deployment/image-lifecycle.md) foram inventariadas sem atualização automática apenas por estarem disponíveis.
...
- [ ] Um [teste de restauração completo](../operations/backup-and-recovery.md) foi executado e cronometrado em ambiente isolado.
...
Use o [runbook de manutenção e mudanças](../operations/maintenance-runbook.md) para preencher baseline, responsáveis, janela, etapas, critérios objetivos, log de timestamps, rollback, observação posterior e fechamento.
...
- [Validação pós-instalação](../k8s/k3s/operations/checklist.md)
- [Backup, atualização e remoção de nós](../k8s/k3s/operations/cluster-maintenance.md)
- [Longhorn e armazenamento persistente](../k8s/extensions/longhorn.md)
- [Templates de GitOps e monitoramento](../strategies/deployment/templates.md)
- [Argo CD](../strategies/deployment/argo-cd.md) e [bootstrap GitOps](../strategies/deployment/gitops-bootstrap.md)
- [Identidade e RBAC](../k8s/security/identity-and-rbac.md)
- [NetworkPolicy](../k8s/networking/network-policies.md)
- [Prontidão de workloads para produção](../k8s/workloads/production-readiness.md)
- [Ciclo de vida de imagens](../strategies/deployment/image-lifecycle.md)
- [Observabilidade e alertas](../operations/observability-and-alerting.md)
- [Backup e recuperação](../operations/backup-and-recovery.md)
- [Runbook de manutenção e mudanças](../operations/maintenance-runbook.md)
