...
Um servidor com agentes distribui workloads, mas mantém um único nó de control plane. Para HA com etcd embarcado, siga a orientação de quorum do [planejamento do cluster](../k8s/k3s/planning.md) e não encerre a expansão com apenas dois servidores.
...
2. Leia os [conceitos do Kubernetes](../k8s/concepts.md) e a [arquitetura do K3s](../k8s/k3s/architecture.md).
3. Siga o [planejamento do cluster](../k8s/k3s/planning.md) para definir nomes, endereços, endpoint da API, quantidade de servidores e custódia do token.
...
**Obrigatória:** configure o [firewall básico dos hosts](../security/hosts/firewall.md), aplique as [regras de rede dos nós K3s](../k8s/k3s/networking/host-firewall.md) de acordo com a topologia e prepare as [ferramentas de linha de comando](../reference/command-line-tools.md) na estação que administrará o cluster.

**Recomendada:** aplique o [hardening do SSH](../security/hosts/ssh.md) e configure o [Fail2Ban](../security/hosts/fail2ban.md) conforme o modelo de acesso aos hosts.

**Condicional:** consulte [portas publicadas pelo Docker](../security/hosts/docker-published-ports.md) somente nos hosts em que Docker estiver instalado e houver containers com portas publicadas. Essa etapa não é um requisito do K3s.
...
Execute o passo a passo do [primeiro servidor K3s](../k8s/k3s/cluster/first-server.md). Essa mesma etapa encerra a instalação do perfil de nó único e inicializa o control plane dos perfis multinó.
...
1. Configure o [acesso remoto ao cluster](../k8s/security/remote-access.md).
2. Mantenha o kubeconfig administrativo restrito e use [identidade, autenticação e RBAC](../k8s/security/identity-and-rbac.md) quando outras pessoas ou automações não precisarem de acesso total.
...
Siga a seção de snapshot em [backup, atualização e remoção](../k8s/k3s/operations/cluster-maintenance.md). Guarde externamente o snapshot e o token, considerando separadamente os dados de volumes quando eles passarem a existir.
...
- **Um servidor com agentes:** siga [adicionar agente](../k8s/k3s/cluster/add-agent.md) em cada worker desejado.
- **HA com etcd embarcado:** siga [adicionar servidor](../k8s/k3s/cluster/add-server.md) até obter pelo menos três servidores em quantidade ímpar. Depois, [adicione agentes](../k8s/k3s/cluster/add-agent.md) somente se quiser separar ou ampliar a capacidade de execução dos workloads.
...
- se a escolha for Longhorn, siga a página do [Longhorn](../k8s/extensions/longhorn.md), incluindo a preparação dos nós e as verificações indicadas nela.
...
**Opcional.** Configure [Gateway API e Traefik](../k8s/networking/gateway-api-and-traefik.md) quando precisar publicar serviços. Instale o [cert-manager](../k8s/extensions/cert-manager.md) quando quiser automatizar a emissão e renovação dos certificados descritos no percurso.
...
1. instale o [Argo CD](../strategies/deployment/argo-cd.md);
2. execute o [bootstrap do repositório GitOps](../strategies/deployment/gitops-bootstrap.md);
3. escolha apenas os [templates copiáveis](../strategies/deployment/templates.md) que serão usados.
...
**Opcional e dependente dos fluxos reais das aplicações.** Revise [NetworkPolicies](../k8s/networking/network-policies.md) antes de ativar isolamento. Se usar os templates GitOps dessa página, faça esta etapa somente depois de concluir o bootstrap do Argo CD e versionar as políticas revisadas.
