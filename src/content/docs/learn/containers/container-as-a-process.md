...
Nenhuma chamada de sistema feita pelo processo dentro do container passa por um kernel diferente: é o mesmo kernel do host que atende, na mesma velocidade, com a mesma versão e os mesmos módulos carregados que qualquer outro processo do host usa. É essa ausência de um segundo kernel que explica por que iniciar um container é quase instantâneo (não existe um boot para esperar) e por que uma vulnerabilidade no kernel do host é, em princípio, uma superfície de ataque compartilhada por todos os containers rodando sobre ele, não isolada por container. A comparação mais completa entre esse modelo e o de uma máquina virtual (que roda seu próprio kernel sobre um hypervisor) está em [máquinas virtuais vs. containers](../../virtualization/vms-vs-containers/); por ora, a distinção essencial é só esta: container isola visão e recursos dentro de um kernel único; VM isola executando um kernel próprio sobre outro.
