...
import restoreVolumeBackupScript from '../../../../../scripts/restore-volume-backup.sh?raw';
import restoreVolumeBackupPvcScript from '../../../../../scripts/restore-volume-backup-pvc.sh?raw';
...
<ScriptHelper
  ...
  script={restoreVolumeBackupScript}
  fields={[
    { var: 'BACKUP_NAME', label: 'Nome do backup (campo metadata.name do Backup)' },
    { var: 'RESTORED_VOLUME_NAME', label: 'Nome do novo volume restaurado' },
  ]}
/>
...
<ScriptHelper
  runWhere="qualquer máquina com `KUBECONFIG` e acesso administrativo à API"
  script={restoreVolumeBackupPvcScript}
  fields={[{ var: 'RESTORED_VOLUME_NAME', label: 'Nome do volume restaurado' }]}
/>
