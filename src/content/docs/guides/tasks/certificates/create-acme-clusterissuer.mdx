...
import dnsCredentialScript from '../../../../../scripts/create-acme-clusterissuer-dns-credential.sh?raw';
import clusterIssuerScript from '../../../../../scripts/create-acme-clusterissuer.sh?raw';
...
```mermaid
sequenceDiagram
    accTitle: Desafio ACME DNS-01
    accDescr: O cert-manager solicita um certificado à Let's Encrypt, que responde com um desafio. O cert-manager cria um registro TXT temporário via API do provedor DNS. A Let's Encrypt consulta esse registro pelos resolvers configurados e, se válido, emite o certificado.

    participant CM as cert-manager
    participant DNS as Provedor DNS
    participant LE as Let's Encrypt

    CM->>LE: Solicita certificado
    LE-->>CM: Desafio DNS-01
    CM->>DNS: Cria registro TXT temporário
    LE->>DNS: Consulta o registro TXT
    DNS-->>LE: Registro válido
    LE-->>CM: Certificado emitido
    CM->>DNS: Remove o registro TXT
```
...
<ScriptHelper
  ...
  script={dnsCredentialScript}
  fields={[{ var: 'DNS_PROVIDER_TOKEN', label: 'Token de API do provedor DNS', type: 'password' }]}
/>
...
<ScriptHelper
  runWhere="qualquer máquina com `KUBECONFIG` e acesso administrativo à API"
  script={clusterIssuerScript}
  fields={[{ var: 'ACME_CONTACT_EMAIL', label: 'E-mail de contato ACME' }]}
/>
