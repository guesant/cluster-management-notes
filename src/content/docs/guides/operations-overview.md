...
**Procedimento:** [prontidão de workloads para produção](../kubernetes/workloads/production-readiness.md). **Evidência mínima:** owner e criticidade registrados, manifesto ou revisão versionada e resultado das validações aplicáveis.
...
**Procedimento:** [ciclo de vida de imagens de containers](deployment/image-lifecycle.md). **Evidência mínima:** tag e digest desejados e observados, origem da mudança, avaliação de compatibilidade, homologação e referência de rollback.
...
**Procedimento:** [prontidão de workloads para produção](../kubernetes/workloads/production-readiness.md). **Evidência mínima:** semântica de cada probe, tempos medidos e resultados dos testes de inicialização, indisponibilidade, travamento e encerramento.
...
- [ ] [Certificados](../kubernetes/extensions/cert-manager.md) e credenciais próximos do vencimento foram identificados.
- [ ] Não surgiram workloads com [tags flutuantes, imagens sem inventário ou versões divergentes](deployment/image-lifecycle.md) do estado desejado.
...
- [ ] Novas versões de K3s, charts, controllers, operators e [imagens](deployment/image-lifecycle.md) foram inventariadas sem atualização automática apenas por estarem disponíveis.
...
- [Validação pós-instalação](../operations/k3s-post-install-checklist.md)
- [Backup, atualização e remoção de nós](../operations/k3s-cluster-maintenance.md)
- [Longhorn e armazenamento persistente](../kubernetes/extensions/longhorn.md)
- [Templates de GitOps e monitoramento](deployment/templates.md)
- [Argo CD](deployment/argo-cd.md) e [bootstrap GitOps](deployment/gitops-bootstrap.md)
- [Identidade e RBAC](../kubernetes/security/identity-and-rbac.md)
- [NetworkPolicy](../kubernetes/networking/network-policies.md)
- [Prontidão de workloads para produção](../kubernetes/workloads/production-readiness.md)
- [Ciclo de vida de imagens](deployment/image-lifecycle.md)
