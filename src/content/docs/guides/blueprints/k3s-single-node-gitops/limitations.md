...
Este blueprint prioriza simplicidade e custo sobre disponibilidade. Cada limitação abaixo é uma consequência direta das [decisões adotadas](../k3s-single-node-gitops/#decisões-adotadas), não um defeito a ser corrigido — para eliminá-las, é necessário migrar para uma topologia diferente (veja "Não coberto" abaixo).
