...
domínios internos (`grafana.cluster.local`) para `127.0.0.1`, e um **reverse proxy** (Nginx,
...
graph LR
    ...
    subgraph After["Com blueprint"]
        ...
        B -->|"https://grafana.cluster.local"| G2["Grafana"]
        B -->|"https://prometheus.cluster.local"| P2["Prometheus"]
        B -->|"https://argocd.cluster.local"| A2["ArgoCD (porta 443 única)"]
    end
```
...
- [Split-horizon DNS](../../../learn/networking/split-horizon-dns/): o conceito por trás de um
  ...
  `grafana.cluster.local` existir só na rede interna.
...
sequenceDiagram
    ...
    Admin->>DNS: resolve grafana.cluster.local
    DNS-->>Admin: 127.0.0.1
    Admin->>Proxy: conexão HTTPS, SNI = grafana.cluster.local
...
```mermaid
graph TB
    Cluster["Dentro do cluster<br/>CoreDNS resolve .cluster.local"]
...
controla. O sufixo `.cluster.local` usado nos exemplos é um domínio arbitrário de uso interno, não
o `.local` reservado por multicast DNS (RFC 6762); qualquer sufixo que não colida com um domínio
público real funciona, desde que seja usado de forma consistente. Nenhuma das duas variantes
