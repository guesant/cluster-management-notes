...
```yaml
Topologia A: 1 servidor + agentes
  [Servidor] (control plane + etcd)
     ↑
     └─→ [Agente 1] (workloads)
     └─→ [Agente 2] (workloads)
     └─→ [Agente N] (workloads)

Topologia B: 3 servidores + agentes (recomendado para HA)
  [Servidor 0] ← quorum etcd ─→ [Servidor 1] ← quorum etcd ─→ [Servidor 2]
     ↑                                ↑                            ↑
     └─→ [Agente 1, 2, ..., N] (apenas executam workloads)
```
