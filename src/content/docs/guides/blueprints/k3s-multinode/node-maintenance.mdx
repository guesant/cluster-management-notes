...
**Flags:**
- `--ignore-daemonsets`: DaemonSets (ex.: Flannel, Longhorn agent) não são drenáveis — ignore-os
- `--delete-emptydir-data`: pods que usam emptyDir (dados temporários) são deletados sem aviso
- `--grace-period=30`: aguarda 30 segundos para pods terminarem gracefully antes de forcibly kill
...
Agora o nó está `CordonSchedulingDisabled` — nenhum novo pod é agendado nele. Você pode:
...
Se o nó está danificado, você pode desligá-lo sem impactar o cluster:

```yaml
# Remover o nó do cluster (não apenas drenar, mas remover)
...
Pod não é gerenciado por Deployment/StatefulSet/DaemonSet — é um pod solo. Solução:
