...
Um cluster K3s multinode exige comunicação entre nós em portas específicas — servidores se coordenam via etcd, agentes conectam à API, e todos precisam falar com flannel (rede overlay). Este guia documenta quais portas abrir, em qual direção e entre quem.
...
```
                             ┌─────────────────────────────┐
                             │ Servidor 0 (líder etcd)     │
                             │ - 6443 (API)                │
                             │ - 2379-2380 (etcd peer)     │
                             └──────────┬──────────────────┘
                                        │
                       TCP 2379-2380    │    TCP 2379-2380
                       (etcd peer)      │    (etcd peer)
                                        │
         ┌──────────────────────────────┼──────────────────────────────┐
         │                              │                              │
         ▼                              ▼                              ▼
    Servidor 1                    Servidor 2                    Agente 1, 2, ...
    TCP 6443 (API)               TCP 6443 (API)               TCP 6443 → Servidores
    TCP 10250                    TCP 10250                    TCP 10250 (kubelet)
    UDP 8472 (flannel)          UDP 8472 (flannel)          UDP 8472 (flannel)
    
    ◄─────────────────────────────────────────────────────────────────►
                  UDP 8472 (VXLAN — overlay network)
                  TCP 10250 (kubelet — node-to-node comms)
```
...
Se seus servidores estão em uma rede (ex.: `10.0.0.0/24`) e agentes em outra (ex.: `10.1.0.0/24`), você precisa encaminhar tráfego entre elas. Faça isso **no seu roteador/gateway**, não no UFW de cada nó:

- Rota: `10.0.0.0/24` ↔ `10.1.0.0/24` nas portas 6443, 10250, 8472
- Se o roteador tiver firewall, adicione as regras lá
...
Depois de configurar firewall, testar conectividade entre nós:
...
Se conectar, firewall está ok.

## Considerações adicionais

### Subredes IPv6

Se está usando IPv6, adicione as mesmas portas para IPv6. UFW aceita:
...
Se nós estão atrás de NAT (ex.: numa nuvem privada), a rede interna (`10.0.0.0/24`) ainda funciona — apenas certifique-se que o firewall internal permite o tráfego. A NAT é **transparente** para Kubernetes contanto que os nós possam se alcançar internamente.

### BGP (se Flannel estiver em modo BGP)

BGP usa a porta **179 TCP** entre nós. Se estiver usando mode BGP em lugar de VXLAN (raro), adicione também:
