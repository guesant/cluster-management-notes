...
A instalação oficial usa o script mantido pelo próprio projeto Helm:
...
Fixe `DESIRED_VERSION` com uma versão específica das [releases do Helm](https://github.com/helm/helm/releases) em vez de omiti-la; sem essa variável, o script instala a versão mais recente disponível no momento da execução, o que torna a instalação não reprodutível entre máquinas.
